  ...
  try {
    const getdata = await db.sequelize.query(
      `select sd.name,sd.phone,sd.email,sd.role,sd.class,sd.section,sd.classteacher,ad.Comment,ad.date,ad.isPersent from school.attendances as ad 
        ...
        where ad.role='Student' && cast(ad.date as date)=cast(now() as date) `,
      {
        //&& ad.date=${date}
...
const getAttendanceTe = async (req, res) => {
  ...
  try {
    const getdata = await db.sequelize.query(
      `select sd.name,sd.phone,sd.email,sd.role,sd.class,sd.classteacher,ad.Comment,ad.date,ad.isPersent,sd.teacherId from school.attendances as ad 
        inner join school.studentdetails as sd on sd.id=ad.userId
        where ad.role='Teacher' && cast(ad.date as date)=cast(now() as date) `,
      {
        type: QueryTypes.SELECT,
      }
    );
...
const getAttendanceByid = async (req, res) => {
  ...
  try {
    const getDataById = await db.sequelize.query(
      `select sd.name,sd.phone,sd.email,sd.role,sd.class,sd.section,sd.classteacher,ad.Comment,ad.date,ad.isPersent from school.attendances as ad 
        inner join school.studentdetails as sd on sd.id=ad.userId
        where ad.userId=${userId} && cast(ad.date as date)=cast(now() as date)`,
      {
        type: QueryTypes.SELECT,
      }
    );
...
const getAttendanceCM= async (req, res) => {
  ...
  try {
    const getdata = await db.sequelize.query(
      `select sd.name,sd.phone,sd.email,sd.role,sd.class,sd.section,sd.classteacher,ad.Comment,ad.date,ad.isPersent from school.attendances as ad 
      inner join school.studentdetails as sd on sd.id=ad.userId
      where ad.userId=${userId} && cast(ad.date as date) BETWEEN  AND cast(now() as date) `,
      {
        type: QueryTypes.SELECT,
      }
    );
...
const getAttendanceSummeryMonthly = async (req, res) => {
  ...
  try {
    const getdata = await db.sequelize.query(
      `select sd.name,sd.phone,sd.email,sd.role,sd.class,sd.section,sd.classteacher,ad.Comment,ad.date,ad.isPersent from school.attendances as ad 
      inner join school.studentdetails as sd on sd.id=ad.userId
      where ad.userId=${userId} && cast(ad.date as date) BETWEEN ${startDate} AND ${EndDate} `,
      {
        type: QueryTypes.SELECT,
      }
    );
